...
let PATH_INIT = path.join(__dirname, "/submissions/");
...
const execute = function (
  ...
  return `docker run --rm -v="${filename}":/${codefileName}/ -v="${testfileName}":/testcase.txt/ -v="${outputfileName}":/output.txt/ -v="${timeMemoryfileName}":/timeMemory.txt/ -e lang=${language} -e outPutFile=output.txt -e timeMemoFile=timeMemory.txt -e probTime=${problem.time} -e probMemo=${problem.memory} online-judge`;
};
          ...
          async.waterfall([
            ...
            function (err, next) {
              ...
              try {
                ...
                let arr = timeMemoryOutput.split("/n");
